...
import json
import torch
from agent.agents.ddpg.ddpg_agent import Agent
...
def train_drone(agent,env,state,score,run):
    ...
    while not env.done:
        ...
        # action in [-1 1] due to htan function
        # the folloiwng line make the action belong [0 1].
        action = (action + 1)/2.0
...
def main():
    ...
    agent = Agent(alpha=0.0001, beta=0.001,
                          input_dims=environment.observation_space.shape, tau=0.001, 
                          batch_size=256, fc1_dims=500, fc2_dims=300, fc3_dims=100, 
                          n_actions=environment.action_space.shape[0], fc_name="_DDPG")
